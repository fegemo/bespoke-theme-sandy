...
function browserifyTask() {
  return browserify({
    standalone: 'bespoke.theme.sandy'
  })
    .add('./lib/bespoke-theme-sandy.js')
    .transform('brfs')
    .bundle()
    .pipe(source('bespoke-theme-sandy.js'))
    .pipe(buffer())
    .pipe(
      header(
        [
    ...
    .pipe(dest('dist'))
    .pipe(rename('bespoke-theme-sandy.min.js'))
    .pipe(uglify())
    .pipe(
      header(
        [
...
function imagesDemoTask() {
  return src('demo/src/imgs/*').pipe(dest('demo/dist/imgs'));
}
...
function devTask() {
  ...
  watch('demo/src/imgs/*', imagesDemoTask);
  watch('demo/src/**/*.css', cssDemoTask);
...
exports.demo = series(
  parallel(browserifyDemoTask, pugDemoTask, imagesDemoTask, cssDemoTask),
  devTask
);
